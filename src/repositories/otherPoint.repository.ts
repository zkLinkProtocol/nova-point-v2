...
export class OtherPointRepository extends BaseRepository<OtherPoint> {
  ...
  > {
    ...
    const result = await transactionManager.query(
      `SELECT address, sum("points") AS "totalPoint" FROM "points" WHERE "createdAt">='${startTime}' AND "createdAt"<'${endTime}' AND type in ('daily_check','invite_box_s2','invite_box_s3') group by address;`
    );
