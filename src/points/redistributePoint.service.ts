...
export class RedistributePointService extends Worker {
  ...
  private async genWithdrawInfoMap() {
    ...
    const withdrawTime = Math.floor((new Date().getTime() - 7 * 24 * 60 * 60 * 1000) / 1000);
    ...
    for (const item of withdrawWeightData) {
      ...
      tokenWithdrawWeightMap.set(
        item.project,
        (tokenWithdrawWeightMap.get(item.project) ?? BigInt(0)) + withdrawBalanceWeighting
      );
      userTokenWithdrawWeightMap.set(
        userTokenMapKey,
        (userTokenWithdrawWeightMap.get(userTokenMapKey) ?? BigInt(0)) + withdrawBalanceWeighting
      );
  ...
  public getTransferFailedPointsWeight() {
    ...
    const [userTokenTransferFailedPointsWeightMap, tokenTransferFailedPointsWeightMap] = transferFailedData.reduce(
      ([userMapResult, tokenMapResult], item) => {
        ...
        const itemTransferWeight =
          BigInt(Number(balance) * 10 ** Number(decimals)) * BigInt(calcTime - transferFailedStartTime);
  ...
  async insertOrUpdateHoldingData(data: Array<OmitEntityTime<UserHolding>>) {
    await this.userHoldingRepository.addManyOrUpdate(
      data,
      ["balance", "pointWeight", "pointWeightPercentage"],
  ...
  async insertOrUpdateStakedData(data: Array<OmitEntityTime<UserStaked>>) {
    await this.userStakedRepository.addManyOrUpdate(
      data,
      ["balance", "pointWeight", "pointWeightPercentage"],
      ["userAddress", "tokenAddress", "poolAddress"]
    );
