  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    while (true) {
      const blockTokenPriceData = await refactorDataSource.query(
        `SELECT * FROM "blockTokenPrice" WHERE "createdAt">='2024-05-30 00:00:00' ORDER BY "blockNumber" asc, "priceId" asc limit ${limit} offset ${offset}`
      );
    ...
    while (true) {
      const blockAddressPointData = await refactorDataSource.query(
        `SELECT * FROM "blockAddressPoint" WHERE "createdAt">='2024-05-30 00:00:00' ORDER BY "blockNumber" asc, "address" asc limit ${limit2} offset ${offset2}`
      );
